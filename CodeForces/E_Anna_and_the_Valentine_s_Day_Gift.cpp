...
int main()
{
    ...
    while (t--)
    {
        ...
        vector<ll> zero_arr(n,0);
        vector<ll> size_arr(n,0);
        for(ll i = 0; i < n;i++) {
            size_arr[i] = a[i].size();
            for(auto it = a[i].rbegin(); it != a[i].rend() and *it == '0';it++) {
                ++zero_arr[i];
            }
        }
        ll ans = 0;
        for(ll i = 0; i < n;i++) {
            ans+= size_arr[i] - zero_arr[i];
        }
        sort(zero_arr.begin(),zero_arr.end());
        reverse(zero_arr.begin(),zero_arr.end());
        for(ll i = 0; i < n;i++) {
            if(i & 1) ans+= zero_arr[i];
        }
        if(ans - 1 >= m) cout<<"Sasha"<<endl;
        else cout<<"Anna"<<endl;
    }
    return 0;
}